        ...
        <div className="px-4 py-6">
          ...
          <div className="pt-6">
            ...
            <div className="space-y-1">
              ...
              <SidebarItem icon={Users} label="Employee" active={true} onClick={() => handleNavigation('/admin/manageemployee')} />
            ...
            {filteredEmployees.map((e) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <img
                    src={e.profilePhoto || "/api/placeholder/50/50"}
                    alt={e.username}
                    className="w-12 h-12 rounded-full"
                  />
                  <div>
                    <div className="font-semibold text-gray-800">{e.username || 'No username'}</div>
                    <div className="text-sm text-gray-500">{e.email || 'No email'}</div>
                    <div className="text-xs text-gray-400">ID: {e._id}</div>
                  </div>
                </div>
                <div className="flex items-center gap-3 text-gray-600">
                  <Eye className="cursor-pointer hover:text-emerald-600" onClick={() => console.log('View', e._id)} />
                  <Edit className="cursor-pointer hover:text-emerald-600" onClick={() => console.log('Edit', e._id)} />
                  <Trash2 className="cursor-pointer hover:text-red-600" onClick={() => console.log('Delete', e._id)} />
                </div>
              </div>
            ))}
          </div>
